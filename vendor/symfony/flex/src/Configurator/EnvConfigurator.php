...
class EnvConfigurator extends AbstractConfigurator
{
    public function configure(Recipe $recipe, $vars, Lock $lock, array $options = [])
    {
        ...
        if (!file_exists($this->options->get('root-dir').'/.env_GogolejokeVoice.test')) {
    ...
    {
        foreach (['.env_GogolejokeVoice.dist', '.env_GogolejokeVoice'] as $file) {
    ...
    {
        foreach (['.env_GogolejokeVoice', '.env_GogolejokeVoice.dist'] as $file) {
            $env = $this->options->get('root-dir').'/'.$file;
            if (!file_exists($env)) {
                continue;
            }
