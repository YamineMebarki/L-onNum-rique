...
class ArticleController extends AbstractController
{
     ...
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function articles(ArticleRepository $repo,  CategoryRepository $repoCategory, Request $request, ObjectManager $manager)
    {
        /* $search = new ArticleSearch();
         $form = $this->createForm(ArticleSearchType::class, $search);
         $form->handleRequest($request);*/
        if($request->request->count() > 0 && $request->request->get('searchOk') == 'Rechercher'){
    ...
    /**
     * @Route("/modif/pass/user/{id}", name="modif_pass")
     */
    public function modifPass(Request $request, User $user,  UserPasswordEncoderInterface $encoder)
    {
        $user = $this->getUser();
        $pass = $user->getPassword();
        if ($request->request->count() > 0) {
            if ($request->request->get('pass')) {
                $oldPass = $request->request->get('pass');
                $user = new User();
                $hash = $encoder->encodePassword($user, $oldPass);
                if ($request->request->get('send') == 'true' &&  $hash == $pass) {
                    return $this->redirectTsoRoute('success');
                } else {
                    return $this->redirectToRoute('home');
                }
            }
    ...
    public function removeUser(User $user)
    {
        ...
        if ($isConnect != null && $isConnect->getRole() === 'admin') {
            ...
            $str =  '' .$user->getUsername().' L@nNumérique vous informe que votre profil à été désactiver,pour plus d\'information veuillez contacter l\'équipe technique. Cordialement ';
    ...
    /**
     * @Route("/delete/user/{id}", name="delete_user_id")
     */
    public function removeUserId(User $user)
    {
        $delete = $this->getDoctrine()
            ->getRepository(User::class)
            ->findOneBy([
        ...
        $mail = htmlspecialchars($user->getEmail());
        $name = htmlspecialchars($user->getUsername());
        $str =  '' .$user->getUsername().' L@nNumérique vous confirme la suppression de votre profil. A bientot ';
        $msg = htmlspecialchars($str);
        $headers = "MIME-Version: 1.0\r\n";
        $headers = "From: L@nNumérique" . PHP_EOL;
        $headers .= 'Content-type: text/html; charset=utf-8' . PHP_EOL;
